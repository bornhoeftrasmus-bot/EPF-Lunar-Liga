...
export async function getOrganisationTeams() {
  ...
  /*
    Small batches avoid one large spike of requests against RankedIn.
    A failed homepage fetch does not remove the rest of the teams.
  */
  const BATCH_SIZE = 5;
  ...
  for (let i = 0; i < candidates.length; i += BATCH_SIZE) {
    ...
    const results = await Promise.allSettled(
      batch.map(async (candidate) => {
        const [homepage, matchesRaw] = await Promise.all([
    ...
    for (const result of results) {
      ...
      /*
        This is the authoritative membership filter.
        Team name is NEVER used to decide whether the team belongs to EPF.
      */
      if (
