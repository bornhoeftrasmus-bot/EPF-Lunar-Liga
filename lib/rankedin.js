...
function parseRankedInDate(value) {
  ...
  // RankedIn sometimes returns Danish/European dates:
  ...
  // ISO and other natively parseable formats.
...
function normalizeMatch(match, index) {
  ...
  const parsedDate = parseRankedInDate(date);
...
function upcomingMatches(matches) {
  ...
  return matches
    .filter((match) => {
      const parsed = parseRankedInDate(match.date);
    ...
    .sort((a, b) => {
      const dateA = parseRankedInDate(a.date);
      const dateB = parseRankedInDate(b.date);
